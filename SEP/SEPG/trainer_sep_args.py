...
class Trainer(object):
    ...
    def preprocess_data(self):
        save_dir = os.path.join(PWD, 'preprocessed')
        save_dir = f"{save_dir}/{args.dataset}"
        if os.path.exists(save_dir):
            with open(f"{save_dir}/edge_feature.jsonl", "r") as f:
                all_edge_feature = json.load(f)
            with open(f"{save_dir}/edgepool_weights.jsonl", "r") as f:
                all_edgepool_weights = json.load(f)
            for i in range(len(self.dataset)):
                self.dataset[i]["edgefeats"] = all_edge_feature[i]
                self.dataset[i]["edgepool_weights"] = all_edgepool_weights[i]
        else:
            all_edge_feature = []
            all_edgepool_weights = []
            for i in range(len(self.dataset)):
                layer_edge_feature = []
                layer_edgepool_weights = []
                self.dataset[i]["edgefeats"] = []
                self.dataset[i]["edgepool_weights"] = []
                for layer in range(self.args.tree_depth-1):
                    size_from = self.dataset[i]["node_size"][layer]
                    size_to = self.dataset[i]["node_size"][layer + 1]
                    assign_edges = self.dataset[i]["edges"][layer + 1]
                    assign_edges = np.array(assign_edges).T
                    edges = self.dataset[i]["graph_mats"][layer]
                    edges = np.array(edges)

                    S = toAssignMatrix(assign_edges, size_from, size_to)
                    A = toAdjMatrix(edges, size_from)
                    CE = connection_entropy(A, S)
                    SAS = S.T @ A @ S
                    edge_feature = np.concatenate([CE[:, :, np.newaxis], CE.T[:, :, np.newaxis], SAS[:, :, np.newaxis]], axis=-1)
                    edges_next_layer = self.dataset[i]["graph_mats"][layer + 1]
                    edge_feature_sequential = np.transpose(edge_feature, (1, 0, 2))[*edges_next_layer]
                    edgepool_matrix = getEdgePoolMatrices(A, S)
                    edgepool_weights = edgepool_matrix[0].toarray().T[*assign_edges]
                    self.dataset[i]["edgefeats"].append(edge_feature_sequential)
                    self.dataset[i]["edgepool_weights"].append(edgepool_weights)
                    layer_edge_feature.append(edge_feature_sequential.tolist())
                    layer_edgepool_weights.append(edgepool_weights.tolist())
                all_edge_feature.append(layer_edge_feature)
                all_edgepool_weights.append(layer_edgepool_weights)
            os.makedirs(save_dir, exist_ok=True)
            with open(f"{save_dir}/edge_feature.jsonl", "w") as f:
                json.dump(all_edge_feature, f)
            with open(f"{save_dir}/edgepool_weights.jsonl", "w") as f:
                json.dump(all_edgepool_weights, f)

        for i in range(len(self.dataset)):
            for layer in range(self.args.tree_depth-1):
                self.dataset[i]["edgefeats"][layer] = torch.tensor(self.dataset[i]["edgefeats"][layer])
                self.dataset[i]["edgepool_weights"][layer] = torch.tensor(self.dataset[i]["edgepool_weights"][layer])
    ...
    def load_model(self):
        model = SEP_ICE(self.args).to(self.args.device)
        return model
    ...
    def organize_val_log(self, train_loss, val_loss, val_acc, fold_number, epoch):
        ...
        if val_acc > self.best_acc:
            torch.save(
                self.model.state_dict(),
                './checkpoints/{}/experiment-{}_fold-{}_seed-{}_best-acc-model.pth'.format(self.log_folder_name, self.exp_name, fold_number, self.args.seed)
            )
            self.best_acc = val_acc
            self.best_acc_loss = val_loss
            self.best_acc_epoch = epoch

    def organize_test_log(self, test_loss, test_acc, test_loss_wrt_acc, test_acc_wrt_acc, t_start, t_end, fold_number):
        ...
        self.overall_results['test_loss_wrt_acc'].append(test_loss_wrt_acc)
        self.overall_results['test_acc_wrt_acc'].append(test_acc_wrt_acc)
    ...
    def train(self):
        ...
        for fold_number in train_fold_iter:
            ...
            checkpoint = torch.load('./checkpoints/{}/experiment-{}_fold-{}_seed-{}_best-acc-model.pth'.format(self.log_folder_name, self.exp_name, fold_number, self.args.seed))
            self.model.load_state_dict(checkpoint)
            test_acc_wrt_acc, test_loss_wrt_acc = self.eval(test_loader)

            self.organize_test_log(test_loss, test_acc, test_loss_wrt_acc, test_acc_wrt_acc, t_start, t_end, fold_number)
    ...
    def set_experiment_name(self):
        ...
        exp_name += "SVD={}".format(self.args.svdpool)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--conv', default='GCN', type=str,
                        choices=['GCN', 'GIN', 'GAT', 'Cheb', 'SAGE', 'GAT2', 'Transformer', "EGAT", "Edgefeat-GAT"],
    ...
    parser.add_argument("--svdpool", action='store_true')
    parser.add_argument("--egat_dropout", type=float, default=0.6)
    parser.add_argument("--egat_alpha", type=float, default=0.2)
    parser.add_argument("--num_ce_aggs", type=int, default=100)
    parser.add_argument("--num_svd_pools", type=int, default=100)
