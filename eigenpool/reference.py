...
class EdgeFeatGATEncoder(GcnEncoderGraph):
    def __init__(self, max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
                 num_pool_matrix=2, num_pool_final_matrix=0, pool_sizes=[4], pred_hidden_dims=[50], concat=True,
                 bn=True, dropout=0.0, mask=1, args=None, use_edgefeat=True, use_simple_gat=False,
                 egat_hidden_dims=[128, 128], egat_dropout=0.6, egat_alpha=0.2, egat_num_heads=[1, 3], device='cpu'):
            ...
            if i == 0:
                ...
                if use_simple_gat:
                    self.egat_list.append(
                        BatchGraphAttentionLayer((hidden_dim * (num_layers - 1) + embedding_dim) * self.num_pool_matrix,
                                                 embedding_dim, self.egat_dropout, self.egat_alpha).to(
                            device))
                else:
                    self.egat_list.append(
                        BatchGATFeatureExtractor((hidden_dim * (num_layers - 1) + embedding_dim) * self.num_pool_matrix,
                ...
                if use_simple_gat:
                    self.egat_list.append(
                        BatchGraphAttentionLayer(self.pred_input_dim * self.num_pool_matrix, embedding_dim,
                                                 self.egat_dropout, self.egat_alpha).to(
                            device))
                else:
                    self.egat_list.append(
                        BatchGATFeatureExtractor(self.pred_input_dim * self.num_pool_matrix, self.edgefeat_dim,
    ...
    def __init__(self, max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
        ...
        if self.num_pool_final_matrix > 0:
            if concat:
                if self.con_final:
                    self.pred_model = self.build_pred_layers(
                        self.pred_input_dim * (len(pool_sizes) + self.num_pool_final_matrix + 1), pred_hidden_dims,
                        label_dim, num_aggs=self.num_aggs)
                else:
                    self.pred_model = self.build_pred_layers(
                        self.pred_input_dim * (len(pool_sizes)) + self.pred_input_dim * self.num_pool_final_matrix,
                        pred_hidden_dims, label_dim, num_aggs=self.num_aggs)
            else:
                self.pred_model = self.build_pred_layers(self.pred_input_dim * self.num_pool_final_matrix,
                                                         pred_hidden_dims, label_dim, num_aggs=self.num_aggs)
        else:
