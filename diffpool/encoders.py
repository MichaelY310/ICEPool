...
from EGAT import *
from GAT import OriginalGraphAttentionLayer
...
class ICE_SoftPoolingGcnEncoder(GcnEncoderGraph):
            ...
            if i == 0:
                ...
                if args.original_GAT:
                    self.egat_list.append(
                        OriginalGraphAttentionLayer((self.hidden_dim * (num_layers - 1) + embedding_dim) * 1, embedding_dim, dropout=self.egat_dropout, heads=self.egat_num_heads[0], concat=False, negative_slope=self.egat_alpha).to(
                            device))
                elif use_simple_gat:
                    self.egat_list.append(
                ...
                if args.original_GAT:
                    self.egat_list.append(
                        OriginalGraphAttentionLayer(self.pred_input_dim * 1, embedding_dim, dropout=self.egat_dropout, heads=self.egat_num_heads[0], concat=False, negative_slope=self.egat_alpha).to(
                            device))
                elif use_simple_gat:
                    self.egat_list.append(
    ...
    def forward(self, x, adj, batch_num_nodes, **kwargs):
        ...
        for i in range(self.num_pooling):
            ...
            # self.assign_tensor = self.assign_pred_modules[i](self.assign_tensor)
            ...
            self.assign_tensor = self.assign_pred_modules[i](self.assign_tensor)
            assign_mask = self.construct_mask(self.assign_tensor.shape[-1], batch_num_nodes // self.pooling_size_real)
            # self.assign_tensor_masked = self.assign_tensor + (assign_mask.transpose(-1, -2)-1) * 1e10
            assign_mask_inf = assign_mask.transpose(1,2).repeat(1, self.assign_tensor.shape[1], 1)
            self.assign_tensor_masked = self.assign_tensor.masked_fill(assign_mask_inf == 0, -1e9)
            self.assign_prob = torch.nn.Softmax(2)(self.assign_tensor_masked * 3) * embedding_mask
            self.assign_tensor = torch.nn.Softmax(2)(self.assign_tensor_masked) * embedding_mask
            ...
            edgefeat_mask = (adj != 0).float()  # 0: no edge
    ...
    def connection_entropy(self, A, S):
        ...
        P = S_A / (STA_extS.unsqueeze(-1) + 1e-10)
    ...
    def __init__(self, max_num_nodes, max_num_clusters_fordet, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
        ...
        for i in range(num_pooling):
            ...
            if i == 0:
                ...
                if args.original_GAT:
                    self.egat_list.append(
                        OriginalGraphAttentionLayer(mul*(self.hidden_dim * (num_layers - 1) + embedding_dim) * 1, embedding_dim, dropout=self.egat_dropout, heads=self.egat_num_heads[0], concat=False, negative_slope=self.egat_alpha).to(
                            device))
                elif use_simple_gat:
                    self.egat_list.append(
