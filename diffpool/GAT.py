...
class OriginalGraphAttentionLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        self.lin_edge = Linear(3, heads * out_channels, bias=False,
                                   weight_initializer='glorot')
    ...
    def forward(self, x: Tensor, edge_attr: Tensor, mask: Optional[Tensor] = None):
        r"""Forward pass.
        Args:
        ...
        edge_attr = self.lin_edge(edge_attr.permute(0, 2, 3, 1)).view(B, N, N, H, C)

        alpha_src = torch.sum(x * self.att_src, dim=-1)  # [B, N, H]
        alpha_dst = torch.sum(x * self.att_dst, dim=-1)  # [B, N, H]
        alpha_edge = torch.sum(edge_attr * self.att_edge, dim=-1)

        alpha = alpha_src.unsqueeze(1) + alpha_dst.unsqueeze(2) + alpha_edge  # [B, N, N, H]

        # Weighted and masked softmax:
        alpha = F.leaky_relu(alpha, self.negative_slope)
        alpha = alpha.masked_fill(adj.unsqueeze(-1) == 0, -1e9)
