...
def evaluate(dataset, model, args, name='Validation', max_num_examples=None):
    ...
    for batch_idx, data in enumerate(dataset):
        ...
        det_edgepool_matrices_dic = Variable(data['det_edgepool_matrices_dic'].float(), requires_grad=False).to(
            args.device)

        ypred = model(h0, adj, batch_num_nodes, batch_num_nodes_after_det_pool=batch_num_nodes_after_det_pool,
                      assign_x=assign_input, det_assign_matrix=det_assign_matrix, det_edgefeat_list=det_edgefeat_list,
                      det_edgepool_matrices_dic=det_edgepool_matrices_dic).to(args.device)
...
def train(dataset, model, args, same_feat=True, val_dataset=None, test_dataset=None, writer=None,
    ...
    for epoch in range(args.num_epochs):
        ...
        if not epoch < 11:
            continue
        for batch_idx, data in enumerate(dataset):
            if epoch == 11 and batch_idx < 69:
                continue

            if (batch_idx == 69 and epoch == 11):
                checkpoint = torch.load('checkpoint_epoch11_batch69.pth', map_location=args.device)
                model.load_state_dict(checkpoint['model_state_dict'])
                optimizer.load_state_dict(checkpoint['optimizer_state_dict'])

            if (batch_idx == 70 and epoch == 11):
                print(1)
            ...
            ypred = model(h0, adj, batch_num_nodes, batch_num_nodes_after_det_pool=batch_num_nodes_after_det_pool,
                          assign_x=assign_input, det_assign_matrix=det_assign_matrix,
                          det_edgefeat_list=det_edgefeat_list, det_edgepool_matrices_dic=det_edgepool_matrices_dic).to(
                args.device)
...
def benchmark_task_val_test(args, writer=None, feat='node-label', log_dir=None):
    print("========== benchmark_task_val ========")
    all_vals = []
    graphs = load_data.read_graphfile(args.datadir, args.bmname, max_nodes=args.max_nodes)
    ...
    for i in range(NUM_TESTS):
        ...
        train_dataset, val_dataset, test_dataset, max_num_nodes, max_num_clusters_fordet, input_dim, assign_input_dim = \
            cross_val.prepare_val_test_data(graphs, args, i, max_nodes=args.max_nodes)
        if args.method == 'ice':
            print('Method: ice')
            model = encoders.ICE_SoftPoolingGcnEncoder(
                max_num_nodes,
...
def arg_parse():
    ...
    parser.add_argument('--edge_pool_weight', dest='edge_pool_weight', type=float,
                        help='Weight of SVD Pool result if not Eigen_concat_Singular')
    ...
    parser.add_argument('--egat_hidden_dims', dest='egat_hidden_dims', type=int, nargs='+',
                        help='Parameter for EGAT layer')
...
def main():
    ...
    if prog_args.bmname is not None:
        ...
        benchmark_task_val_test(prog_args, writer=writer, log_dir=log_dir)
